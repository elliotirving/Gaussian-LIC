    ...
    const bool debug, const bool no_color) 
{
    ...
    if(P != 0) 
    {
        auto tup = CudaRasterizer::Rasterizer::forward(
            ...
            out_final_T.contiguous().data_ptr<float>(),
    ...
    return std::make_tuple(rendered, num_buckets, out_color, out_final_T, out_depth, radii, geomBuffer, binningBuffer, imgBuffer, sampleBuffer);
}
    ...
    const bool debug) 
{
    ...
    {  
        CudaRasterizer::Rasterizer::backward(P, degree, M, R, B,
            ...
            dL_dcolors_precomp.contiguous().data_ptr<float>(),
            dL_dmeans3D.contiguous().data_ptr<float>(),
            dL_dcov3Ds_precomp.contiguous().data_ptr<float>(),
